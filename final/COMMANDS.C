...
int executeCommand(int op, char* command){
	...
	else if(strcmpi(command, "change_prompt") == 0) change_prompt(op);
...
void help(int op){
  ...
  switch(op) {
	...
	write("exit\t\t\t:- exits the fmOS shell.\n");
		...
		write("change_prompt [PROMPT]:- changes the prompt to the specifed string\n\n");
...
void change_prompt(int op){
	...
	switch(op) {
		case EXEC:
			...
			if((newPrompt = strtok(NULL, " ,-|")) != NULL) {
				if(tooManyArgs()) return;
				strcat(newPrompt, " ");
				strcpy(MainShell->prompt, newPrompt);
			} else write("Incorrect command syntax: Try change_prompt [newPrompt]\n");

			 break;
		case HELP:
		if(tooManyArgs()) return;
		  write("'change_prompt [newPrompt]': changes the prompt to the new prompt\n");
		  break;
	}
		...
		case EXEC:
			 ...
			 if((name = strtok(NULL, " ,-|")) != NULL) {
				...
				if(tooManyArgs()) return;
					...
					if(newPriority < -129 || newPriority > 128){
